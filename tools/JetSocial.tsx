...
Generated by JetSuite - ${new Date().toLocaleDateString()}`;
  ...
  const handleDownloadImage = (platform: string, index: number) => {
    const imageData = generatedImages[platform];
    if (!imageData) return;

    const link = document.createElement('a');
    link.href = imageData;
    link.download = `${platform}_Image_${index + 1}_${Date.now()}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    
    setCopySuccess(`${platform} image downloaded!`);
    setTimeout(() => setCopySuccess(''), 2000);
  };
                ...
                {posts.map((post, index) => (
                    <div key={index} className="bg-brand-card p-6 rounded-xl shadow-lg">
                        ...
                        <div className="mt-4 bg-brand-light p-4 rounded-lg border border-brand-border">
                            <h4 className="text-sm font-semibold text-brand-text mb-2">Visual Suggestion #{index + 1}</h4>
                        ...
                        <div className="mt-4 space-y-3">
                          ...
                          <div className="border-t border-brand-border pt-3">
                            <p className="text-xs text-brand-text-muted mb-2">Download Post Assets:</p>
                            <div className="grid grid-cols-2 gap-2">
                              <button
                                onClick={() => handleDownloadText(post, index)}
                                className="bg-green-600 hover:bg-green-700 text-white text-sm font-semibold py-2 px-3 rounded-lg transition duration-300 flex items-center justify-center gap-1"
                              >
                                <span>📄</span> Download Text
                              </button>
                              {generatedImages[post.platform] && (
                                <button
                                  onClick={() => handleDownloadImage(post.platform, index)}
                                  className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold py-2 px-3 rounded-lg transition duration-300 flex items-center justify-center gap-1"
                                >
