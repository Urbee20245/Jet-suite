...
const GbpDetectedCard: React.FC<{ detectedGbp: BusinessSearchResult; isConfirmed: boolean; onConfirm: () => void; onReject: () => void; }> = ({ detectedGbp, isConfirmed, onConfirm, onReject }) => (
    <div className={`mt-6 p-6 rounded-xl border-2 ${isConfirmed ? 'border-green-400 bg-green-50' : 'border-brand-border bg-brand-light'}`}>
        <div className="flex justify-between items-start">
            <h3 className="text-lg font-bold text-brand-text">Google Business Profile Detected</h3>
            {isConfirmed ? (
                 <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">✓ Confirmed</span>
            ) : (
                <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">✓ Auto-Detected</span>
            )}
        </div>
        <p className="text-sm text-brand-text-muted mt-1 mb-4">We found this profile linked on your website. Please confirm if it's correct.</p>
        <div className="bg-white p-4 rounded-lg border border-brand-border">
            <h4 className="font-bold text-brand-text">{detectedGbp.name}</h4>
            <div className="flex items-center text-sm mt-1"><StarIcon className="w-4 h-4 text-yellow-400"/><span className="ml-1 font-semibold">{detectedGbp.rating}</span><span className="text-brand-text-muted ml-1">({detectedGbp.reviewCount} reviews)</span></div>
            <p className="text-sm text-brand-text-muted mt-1">{detectedGbp.address}</p>
        </div>
        {!isConfirmed && (
            <div className="mt-4 flex items-center gap-4">
                <button onClick={onConfirm} className="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded-lg">✓ This is correct</button>
                <button onClick={onReject} className="text-sm font-semibold text-brand-text-muted hover:underline">Not my business</button>
            </div>
        )}
    </div>
);
...
export const BusinessDetails: React.FC<BusinessDetailsProps> = ({ profileData, onUpdate, setActiveTool }) => {
