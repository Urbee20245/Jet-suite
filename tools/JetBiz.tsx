...
export const JetBiz: React.FC<JetBizProps> = ({ tool, addTasksToGrowthPlan, onSaveAnalysis, profileData, setActiveTool, growthPlanTasks, onTaskStatusChange, userId, activeBusinessId }) => {
  ...
  const runAnalysis = async (business: ConfirmedBusiness) => {
    ...
    try {
      ...
      console.log('🚀 [JetBiz] Automatically adding', newTasks.length, 'tasks to Growth Plan...');
      await addTasksToGrowthPlan(newTasks);
      console.log('✅ [JetBiz] Tasks added successfully to Growth Plan');
      
      onSaveAnalysis(analysis);
    } catch (err) { 
      setError('Failed to get analysis. Please try again.'); 
      console.error('❌ [JetBiz] Analysis failed:', err);
    } 
    finally { 
