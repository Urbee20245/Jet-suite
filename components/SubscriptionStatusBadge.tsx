 ...
 * - Expands on hover to show text label.
 */
export const SubscriptionStatusBadge: React.FC<SubscriptionStatusBadgeProps> = ({ userId }) => {
  ...
  if (status === 'loading') {
    return (
      <div className="h-8 w-8 bg-slate-800/50 animate-pulse rounded-full border border-slate-700"></div>
    );
  }

  // Active/Paid State (Green Glow)
  if (status === 'active') {
    const label = isAdminGranted ? 'Active Access' : 'Active Plan';
    return (
      <div 
        className="group flex items-center gap-0 hover:gap-2 pl-2 pr-2 hover:pr-3 py-2 bg-green-500/10 border border-green-500/30 rounded-full shadow-[0_0_15px_rgba(34,197,94,0.2)] transition-all duration-300 cursor-pointer"
        title={label}
      >
        ...
        <div className="grid grid-cols-[0fr] group-hover:grid-cols-[1fr] transition-all duration-300">
            <span className="overflow-hidden text-[10px] sm:text-xs font-bold text-green-500 uppercase tracking-wider whitespace-nowrap">
              {label}
            </span>
        </div>
      </div>
  ...
  return (
    <div 
      className="group flex items-center gap-0 hover:gap-2 pl-2 pr-2 hover:pr-3 py-2 bg-blue-500/10 border border-blue-500/30 rounded-full shadow-[0_0_15px_rgba(59,130,246,0.2)] transition-all duration-300 cursor-pointer" 
      title="Free Account"
    >
      <div className="relative flex h-2 w-2 flex-shrink-0">
      ...
      <div className="grid grid-cols-[0fr] group-hover:grid-cols-[1fr] transition-all duration-300">
        <span className="overflow-hidden text-[10px] sm:text-xs font-bold text-blue-400 uppercase tracking-wider whitespace-nowrap">
          Free Account
        </span>
      </div>
    </div>
