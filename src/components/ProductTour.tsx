  ...
  const startTour = () => {
    ...
    const driverObj = driver({
      ...
      steps: [
        ...
        {
          element: '[data-tour="businessdetails"]',
          popover: {
        ...
        {
          element: '[data-tour="growthscore"]',
          popover: {
            title: '📊 Growth Score (0-99)',
            description: 'Your Growth Score increases as you complete tasks and improve your online presence. Track your progress here and compete with yourself!',
            side: 'right',
            align: 'start'
          }
        },
  ...
  const saveTourCompletionToDatabase = async (userId: string) => {
    try {
      ...
      await supabase
        ...
        .upsert({
          user_id: userId,
          preferences: { tour_completed: true, tour_completed_at: new Date().toISOString() }
        }, { onConflict: 'user_id' });
    } catch (error) {
